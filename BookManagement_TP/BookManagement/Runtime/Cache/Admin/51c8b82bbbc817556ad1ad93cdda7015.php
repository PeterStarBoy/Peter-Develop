...
<form name="form1" method="post" action="">
   ...
   <tr>
     <td valign="top"><table width="100%" border="0" cellpadding="02" cellspacing="2" bordercolor="#E3F4F7">
       <tr>
         <td height="33" background="/BookManagement/Public/Admin/image/bookgh.gif">&nbsp;</td>
       </tr>
       <tr>
         <td valign="top" bgcolor="#D2E6F1">
<!--
<?php  include("conn/conn.php"); $sql=mysql_query("select borr.id as borrid,borr.borrowTime,borr.backTime,borr.ifback,r.*,t.name as typename,t.number,book.bookname,book.price,pub.pubname,bc.name as bookcase from tb_borrow as borr join tb_reader r on borr.readerid=r.id join tb_readerType t on r.typeid=t.id join tb_bookinfo as book on book.id=borr.bookid join tb_publishing as pub on book.ISBN=pub.ISBN  join tb_bookcase as bc on book.bookcase=bc.id where r.barcode='".$_POST[barcode]."' and borr.ifback=0"); $info=mysql_fetch_array($sql); ?>
-->
       ...
       <tr>
         <td valign="top" bgcolor="#D2E5F1"><table width="100%" height="35" border="1" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF" bordercolorlight="#FFFFFF" bordercolordark="#D2E3E6" bgcolor="#FFFFFF">
				...
				<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$valdata): $mod = ($i % 2 );++$i;?><tr>
                     ...
                     <td width="12%" align="center"><a href="/index.php/Admin/Borrow/bookBack/borrid/<?php echo ($valdata['borrid']); ?>/barcode/<?php echo ($valdate['barcode']); ?>">归还</a>&nbsp;</td>
